    ...
    .then(cached => {
      ...
      return _getTotals(owner, portfolio, range)
        ...
        .then(totals => {
          const allTotals = {};
          const count = {};
          totals.forEach(total => total.forEach(tick => _allTotalsUpdate(allTotals, tick, count)));

          // remove unfull last tick for multiportfolios
          const lastTick = Object.keys(count)[Object.keys(count).length-1];
          if (count[lastTick] !== totals.length) delete allTotals[lastTick];

          // remove zero values
          Object.keys(allTotals).forEach(tick => {
            if (allTotals[tick] === 0 || allTotals[tick].avg === 0) delete allTotals[tick];
          });
...
const _allTotalsUpdate = (allTotals, tick, count) => {
  if (!count[tick.time]) count[tick.time] = 1;
  else count[tick.time] += 1;
